...
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_layer_size=5, output_size=1, window=10):
        """
        input_size: Corresponds to the number of features in the input. Though our sequence length is n, for each month we have only 1 value i.e. total number of passengers, therefore the input size will be 1.
    ...
    def forward(self, input_seq):
        ...
        #predictions = lstm_out.view(len(input_seq), -1)
        #return predictions[-self.window_size:].view(-1)
